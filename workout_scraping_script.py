...
for i in range(1): #30
    ...
    instructions_list.append({"wait": 500})
...
prompt = {"role": "system", "content": '''
    ...
    If the workout plan refers to something else on the page (like a chart) make sure to include it in the workout plan. Extract the workout plans from the new message and generate ones with a standardized structure.
    '''}
...
while len(workouts)< TOTAL_WORKOUTS:
    ...
    if num_tokens_from_string(cleantext, 'cl100k_base')>1000:
        multipart = True
        num_parts = math.ceil(num_tokens_from_string(cleantext, 'cl100k_base')/1000)
        queries=[]
    ...
    if multipart:
        ...
        for query in queries:
            ...
            model="gpt-4o",
            messages=[{"role": "system", "content": 'This is the next part of the entry: {}'.format(query)}
            ], response_model=plans, max_tokens=4096
            )
            resp_results.append(resp2.model_dump()['properties'])
    else:
        resp = clientOA.chat.completions.create(
        model="gpt-4o",
        messages=[prompt,
